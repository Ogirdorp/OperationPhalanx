...
def test():
	run("make_loads")
	dest = os.path.join(cfg.mpmissions_folder, 'test.Takistan')
	dest2 = os.path.join(cfg.missions_folder, 'test.Takistan', 'loads')

	#print dest
	#TODO make this check safer
	if os.path.isdir(dest) and dest != 'test.Takistan':
		shutil.rmtree(dest)
	shutil.copytree('test.Takistan', dest)
...
# def debug():
	# mish = Reader('RisingScimitar.Takistan/mission.sqm').read()
